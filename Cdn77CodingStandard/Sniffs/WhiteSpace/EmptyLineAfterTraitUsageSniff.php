...
final class EmptyLineAfterTraitUsageSniff implements Sniff
{
    ...
    {
        if (UseStatementHelper::isAnonymousFunctionUse($file, $pointer)
        ...
        $nextPtr = TokenHelper::findFirstTokenOnNextLine($file, $pointer);
        if ($tokens[$nextPtr]['content'] === "\n") {
            return; // there is a new line as expected
        }

        $nextPtr = TokenHelper::findNextEffective($file, $nextPtr);
        if ($tokens[$nextPtr]['code'] === T_USE) {
            return; // multiple trait uses, skip to the next one
        }

        $useScopeCondition = $tokens[$pointer]['scope_condition'] ?? false;
        if ($useScopeCondition && isset($tokens[$nextPtr]['conditions'][$useScopeCondition])) {
